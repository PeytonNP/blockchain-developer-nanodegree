...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(msg.sender == _owner, "Error...only the owner can use this functionality.");
        _;
    }
...
contract Pausable is Ownable {
    ...
    function setPaused(bool pausedVar) public onlyOwner {
        _paused = pausedVar;
    }

    // Adding public getter in case its needed
    ...
    modifier whenNotPaused() {
        require(!_paused, "Error..._paused is false");
        _;
    }

    modifier paused() {
        require(_paused, "Error..._paused is true");
        _;
    }
    ...
    function _doPause() internal whenNotPaused {
        _paused = true;
        emit Paused(msg.sender);
    }

    function _doUnpause() internal paused {
        _paused = false;
...
contract ERC165 {
    ...
    function _registerInterface(bytes4 interfaceId) internal {
        require(interfaceId != 0xffffffff, "error in _registerInterface");
...
contract ERC721 is Pausable, ERC165 {
    ...
    function ownerOf(uint256 tokenId) public view returns (address) {
        ...
        require(_tokenOwner[tokenId] != address(0), "There is no owner for the given tokenId");
    ...
    function setApprovalForAll(address to, bool approved) public {
        require(to != msg.sender, "error in setApprovalForAll, to is not msg sender");
    ...
    function transferFrom(address from, address to, uint256 tokenId) public {
        require(_isApprovedOrOwner(msg.sender, tokenId), "isApprovedOrOwner not true");
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public {
        ...
        require(_checkOnERC721Received(from, to, tokenId, _data), "error cannot safeTransferFrom");
    }
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        if (_exists(tokenId) || to == address(0)) {
            revert("The tokenId already exists or the given address is invalid");
        }
...
contract ERC721Enumerable is ERC165, ERC721 {
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view returns (uint256) {
        require(index < balanceOf(owner), "error tokenOfOwnerByIndex index less than balance");
    ...
    function tokenByIndex(uint256 index) public view returns (uint256) {
        require(index < totalSupply(), "error tokenByIndex index less than total supply");
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        require(_exists(tokenId), "error tokenURI does not exist");
